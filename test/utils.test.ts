...
import { milliseconds2text, text2lines } from "../src/utils";
...
describe("test milliseconds2text", () => {
  it("converts 10ms", () => {
    const text = milliseconds2text(
      timeConfig2milliseconds({
        milliseconds: 10,
      }),
    );
...
describe("test text2lines", () => {
  it.each([
    ...
    'breaks text="%s" into lines of length <= 10 when line break is 10',
    (text) => {
      const lineBreak = 10;
