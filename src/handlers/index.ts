...
import {
  handleBearFactInteraction,
  handleGikosayInteraction,
  handleOpenBearBoxInteraction,
  handlePinInteraction,
  handleStapleInteraction,
  handleTakeInteraction,
} from "./interactions";
  ...
  while (true) {
    ...
    for (const key of listPinsRes.keys) {
      ...
      if (timestamp < now) {
        ...
        const unpinMessageRes = await fetch(
