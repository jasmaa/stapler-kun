...
export async function handlePinInteraction(
  message: any,
  env: Env,
): Promise<JsonResponse> {
  const channelId = message.channel_id;
  const getMessagesRes = await fetch(
    `${DISCORD_API_BASEURL}/channels/${channelId}/messages`,
    {
      headers: {
  ...
  const pinMessageRes = await fetch(
    `${DISCORD_API_BASEURL}/channels/${channelId}/pins/${messageId}`,
    {
      method: "put",
      headers: {
        "Content-Type": "application/json",
...
export async function handleTakeInteraction(
  message: any,
  env: Env,
): Promise<JsonResponse> {
...
export async function handleStapleInteraction(
  message: any,
  env: Env,
): Promise<JsonResponse> {
...
export async function handleBearFactInteraction(
  message: any,
  env: Env,
): Promise<JsonResponse> {
...
export async function handleGikosayInteraction(
  message: any,
  env: Env,
): Promise<JsonResponse> {
  const lineBreak = 35;
  ...
  const text = message.data.options.find(
...
export async function handleOpenBearBoxInteraction(
  message: any,
  env: Env,
  workerUrl: string,
): Promise<JsonResponse> {
